...
# Lógica de escenarios validada
es_gran_empresa = facturacion > 20000000
if es_gran_empresa:
    escenarios = [{"nombre": "Límite Gran Empresa", "pct": 0.15}]
    st.warning("🏢 Gran Empresa detectada: Límite al 15%.")
else:
    escenarios = [{"nombre": "Escenario Estándar", "pct": 0.25}, {"nombre": "Escenario Intensivo", "pct": 0.50}]
    st.info("🏭 Pyme detectada: Mostrando escenarios al 25% y 50%.")

st.divider()

# --- 2. CÁLCULOS DE INVERSIÓN ---
...
for i, esc in enumerate(escenarios):
    with cols[i]:
        ...
        inv_optima = techo_deducc / 1.20 # Inversión + 20% = Deducción
        ...
        st.metric("Beneficio Cliente", f"{(inv_optima * 0.20):,.2f} €")

st.divider()

# --- 3. IMPACTO FINAL Y REGISTRO ---
st.header("📉 Simulación Final e Impacto")
monto_final = st.number_input("Inversión Final Acordada (€)", min_value=0.0, value=inv_optima)
...
if st.button("🚀 GUARDAR EN EXPEDIENTES"):
    try:
        # Forzamos la lectura de la primera pestaña (donde moviste EXPEDIENTES)
        ...
        st.balloons()
        st.success("✅ ¡CONECTADO! Datos guardados en el Excel.")
    except Exception as e:
        st.error(f"Error 401: Google rechaza la llave. {e}")
